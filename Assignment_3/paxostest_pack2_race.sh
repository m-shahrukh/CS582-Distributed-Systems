...
PAXOS_TEST=$GOPATH/bin/paxostest_pack2
...
# Start paxos node.
go run -race ${PAXOS_NODE} -myport=${NODE_PORT0} -ports=${ALL_PORTS} -N=4 -id=0 -pxport=${PROXY_PORT} -proxy=0 -retries=10 &
PAXOS_NODE_PID0=$!
sleep 1

go run -race ${PAXOS_NODE} -myport=${NODE_PORT1} -ports=${ALL_PORTS} -N=4 -id=1 -pxport=${PROXY_PORT} -proxy=0 -retries=10 &
PAXOS_NODE_PID1=$!
sleep 1

go run -race ${PAXOS_NODE} -myport=${NODE_PORT2} -ports=${ALL_PORTS} -N=4 -id=2 -pxport=${PROXY_PORT} -proxy=0 -retries=10 &
PAXOS_NODE_PID2=$!
sleep 1

go run -race ${PAXOS_NODE} -myport=${NODE_PORT3} -ports=${ALL_PORTS} -N=4 -id=3 -pxport=${PROXY_PORT} -proxy=0 -retries=10 &
PAXOS_NODE_PID3=$!
sleep 1
