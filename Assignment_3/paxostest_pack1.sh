...
# Start paxos node.
${PAXOS_NODE} -myport=${NODE_PORT0} -ports=${ALL_PORTS} -N=5 -id=0 -pxport=${PROXY_PORT} -proxy=1 -retries=10 2> /dev/null &
PAXOS_NODE_PID0=$!
sleep 1

${PAXOS_NODE} -myport=${NODE_PORT1} -ports=${ALL_PORTS} -N=5 -id=1 -pxport=${PROXY_PORT} -proxy=1 -retries=10 2> /dev/null &
PAXOS_NODE_PID1=$!
sleep 1

${PAXOS_NODE} -myport=${NODE_PORT2} -ports=${ALL_PORTS} -N=5 -id=2 -pxport=${PROXY_PORT} -proxy=1 -retries=10 2> /dev/null &
PAXOS_NODE_PID2=$!
sleep 1

${PAXOS_NODE} -myport=${NODE_PORT3} -ports=${ALL_PORTS} -N=5 -id=3 -pxport=${PROXY_PORT} -proxy=1 -retries=10 2> /dev/null &
PAXOS_NODE_PID3=$!
sleep 1

${PAXOS_NODE} -myport=${NODE_PORT4} -ports=${ALL_PORTS} -N=5 -id=4 -pxport=${PROXY_PORT} -proxy=1 -retries=10 2> /dev/null &
PAXOS_NODE_PID4=$!
sleep 1
